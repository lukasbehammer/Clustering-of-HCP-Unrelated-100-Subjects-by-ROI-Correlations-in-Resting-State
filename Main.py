...
def plot_in_orientation(img_data, orientation, slice, cmap="gray", ax=None, **kwargs):
    if orientation == "coronal":
        ...
        plot = ax.imshow(coronal[:, :, slice], cmap=cmap, **kwargs) if ax else plt.imshow(coronal[:, :, slice], cmap=cmap)
    elif orientation == "sagittal":
        sagittal = np.transpose(img_data, [1, 2, 0])
        sagittal = np.rot90(sagittal, 1)
        plot = ax.imshow(sagittal[:, :, slice], cmap=cmap, **kwargs) if ax else plt.imshow(sagittal[:, :, slice], cmap=cmap)
    elif orientation == "transversal":
        transversal = np.transpose(img_data, [0, 1, 2])
        transversal = np.rot90(transversal, 1)
        plot = ax.imshow(transversal[:, :, slice], cmap=cmap, **kwargs) if ax else plt.imshow(transversal[:, :, slice], cmap=cmap)
    else:
